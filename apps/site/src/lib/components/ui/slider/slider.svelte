<script lang="ts">
	...
	function meterThumbOffset(index: number): number {
		const currentValue = Array.isArray(value) ? value[index] : value;
		if (typeof currentValue !== 'number' || max <= min) return 0;

		const progress = Math.min(Math.max((currentValue - min) / (max - min), 0), 1);
		return -8 * Math.min(progress / 0.05, 1);
	}
</script>
	...
	{#snippet children({ thumbItems })}
		...
		>
			<SliderPrimitive.Range
				...
				class={cn(
					...
					isMeter
						? 'z-10 min-w-5 rounded-sm bg-background shadow-[0px_0px_0px_1px_rgba(0,0,0,0.04),0_1px_1px_rgba(0,0,0,0.05),0_2px_2px_rgba(0,0,0,0.05),0_2px_4px_rgba(0,0,0,0.05)] dark:bg-card/64 dark:inset-shadow-[0_1px_rgb(255_255_255/0.15)]'
						: 'bg-primary'
				)}
			/>
			{#if isMeter}
		...
		{#each thumbItems as thumb (thumb.index)}
			<SliderPrimitive.Thumb
				...
				style={isMeter ? `margin-inline-start: ${meterThumbOffset(thumb.index)}px;` : undefined}
				class={cn(
					isMeter
						? 'z-30 block h-5 w-1 shrink-0 rounded-full bg-muted-foreground shadow-sm ring-ring/50 transition-[background-color,box-shadow] select-none group-hover/slider:bg-foreground/70 after:absolute after:-inset-x-4 after:-inset-y-1 focus-visible:bg-foreground focus-visible:ring-3 focus-visible:outline-hidden active:bg-foreground disabled:pointer-events-none disabled:opacity-50'
